...
serve(async (req) => {
  ...
  try {
    // Parse request body as text to avoid issues with multiple parsing attempts
    ...
    // Using a simpler prompt with fewer parameters
    const toonifyPrompt = `Transform this photo into a Studio Ghibli style cartoon.`;

    console.log('Sending request to Gemini API');
    
    // Using the gemini-2.0-flash-exp-image-generation model with minimal parameters
    ...
    if (!generationResult || !generationResult.candidates || generationResult.candidates.length === 0) {
      console.error('Invalid response from Gemini API:', JSON.stringify(generationResult));
