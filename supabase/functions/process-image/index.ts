...
async function processWithGemini(imageResponse) {
  ...
  // Prepare the content parts exactly as shown in the example
  const contents = [
  ...
  try {
    ...
    const result = await model.generateContent({
      contents: contents,
      // Note: removed responseModalities config as it's not needed with the correct API call structure
    });
    ...
    if (result.response && result.response.candidates && 
      ...
      console.log("Response has candidates structure");
      ...
      console.log("No image found in parts:", 
          JSON.stringify(result.response.candidates[0].content.parts).substring(0, 200) + "...");
    } else {
